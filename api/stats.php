...
try {
    if ($method === 'GET') {
        ...
        // Análises pendentes - CORRECTED: use English status 'pending'
        ...
        // Análises aprovadas - CORRECTED: use English status 'approved'
        $stmt = $db->query("SELECT COUNT(*) as total FROM credit_analysis WHERE status = 'approved'");
        ...
        // Ranking de funcionários (por agendamentos confirmados) - CORRECTED: join on confirmed_by
        $stmt = $db->query("
            SELECT 
                e.id,
                e.name,
